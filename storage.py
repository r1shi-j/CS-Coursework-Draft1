...
class Database:
    ...
    def __init__(self, db: tuple[str, bool]):
        ...
        try:
            if db[1]:
                ...
                if os.path.isfile(db[0]):
                    print(f"Database {db[0]} already exists")
                # if it doesnt exist then say we are creating it
                else:
                    print("Creating database: ", db[0])
                # open the database
                open_db()
            ...
            print(
                "Database not found. If you intend to create a new database file run `python3 main.py new 'my_new_db_name.db'`"
            )
            exit()
    ...
    def sort_tournaments(
        ...
        def quick_sort(
            ...
            key_func: Callable[[tuple[str, str, int, str]], str],
        ) -> list[tuple[str, str, int, str]]:
        ...
        if options[0] == "Date":
            ...
            sorted_winners = quick_sort(
                with_winners,
                lambda x: self.read_tournament_winner(x[0])[1],
            )
    ...
    def create_tournament(
        self, t_id: str, date: str, p_count: int, ttype_id: str
    ):
        self.cursor.execute(
            "INSERT INTO Tournament (tournament_id, date, player_count, tournament_type_id) VALUES (?, ?, ?, ?)",
            (t_id, date, p_count, ttype_id),
        )
        self.connection.commit()

    # creating the grand prixs for a new tournament and adding the respective players to them
    def create_gps_for_tournament(
        self, t_id: str, players: list[tuple[str, str, str, int]]
    ):
        starter_ids = [
            create_uuid(),
            create_uuid(),
        ...
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (starter_ids[0], t_id, 1, False, 1, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (starter_ids[1], t_id, 1, False, 2, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (starter_ids[2], t_id, 1, True, 1, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (starter_ids[3], t_id, 1, True, 2, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (create_uuid(), t_id, 2, False, 1, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (create_uuid(), t_id, 2, True, 1, 2),
        )
        self.cursor.execute(
            "INSERT INTO GrandPrix (grandprix_id, tournament_id, round, inverse, bracket, continuers) VALUES (?, ?, ?, ?, ?, ?)",
            (create_uuid(), t_id, None, None, None, None),
        )
        self.connection.commit()
    ...
    def update_tournament(
        ...
        try:
            self.cursor.execute(
                "UPDATE Tournament SET date = ?, player_count = ?, tournament_type_id = ? WHERE tournament_id = ?",
                (date, p_count, ttype_id, t_id),
            )
            self.connection.commit()
        except sqlite3.IntegrityError:
            print(
                f"Failed to update tournament, either Tournament ID {t_id} not found or Tournament Type ID {ttype_id} not found."
            )
            return

    # reading all the tournament types
    def read_tournament_types(
        self,
    ...
    def read_tournament_type(self, t_id: str) -> str:
        self.cursor.execute(
            "SELECT tournament_type_id FROM Tournament WHERE tournament_id = ?;",
            (t_id,),
        )
        # trying to find it
        try:
    ...
    def read_tournament_accounts(
        self, t_id: str
    ) -> list[tuple[str, str, str, str]]:
        self.cursor.execute(
            "SELECT * FROM Account WHERE tournament_id = ?;",
            (t_id,),
        )
        return self.cursor.fetchall()
    ...
    def create_account(
        self, t_id: str, username: str, password: str
    ) -> bool:
        ...
        try:
            ...
            self.cursor.execute(
                ...
                (create_uuid(), t_id, username, hashed),
            )
            self.connection.commit()
    ...
    def attempt_login(
        ...
        self.cursor.execute(
            "SELECT password_hash FROM Account WHERE tournament_id = ? AND username = ?",
            (t_id, username),
        )
    ...
    def add_player_to_tournament(self, t_id: str, p_id: str):
        try:
            self.cursor.execute(
                "INSERT INTO TournamentParticipation (tournament_id, player_id, tournament_result) VALUES (?, ?, ?)",
                (t_id, p_id, None),
            )
            self.connection.commit()
        except sqlite3.IntegrityError:
            print(
                f"Failed to update tournament, either Tournament ID {t_id} not found or Player ID {p_id} not found."
            )
            return

    # removing a player from a tournament
    def remove_player_from_tournament(
        self, t_id: str, p_id: str
    ) -> bool:
        # try except to check if player can be safely deleted from tournament participation
        try:
            self.cursor.execute(
                "DELETE FROM TournamentParticipation WHERE tournament_id = ? AND player_id = ?;",
                (t_id, p_id),
            )
            self.connection.commit()
    ...
    def read_grand_prix_players(
        self, gp_id: str
    ) -> list[tuple[str, str, str, int]]:
        self.cursor.execute(
            "SELECT * FROM Player WHERE player_id IN (SELECT player_id FROM GrandPrixParticipation WHERE grandprix_id = ?)",
            (gp_id,),
        )
        return self.cursor.fetchall()
    ...
    def create_race(
        ...
        self.cursor.execute(
            "INSERT INTO Race (race_id, grandprix_id, circuit_id) VALUES (?, ?, ?)",
            (r_id, gp_id, c_id),
        )
        for p in player_results:
            # adding the players to the race via RaceParticipation
            self.cursor.execute(
                "INSERT INTO RaceParticipation (race_id, player_id, race_result) VALUES (?, ?, ?)",
                (r_id, p[0], p[1]),
            )
        self.connection.commit()
    ...
    def get_race_count_in_gp(self, gp_id: str) -> int:
        self.cursor.execute(
            "SELECT COUNT(*) FROM Race WHERE grandprix_id = ?",
            (gp_id,),
        )
        return self.cursor.fetchone()[0]
    ...
    def get_player_count_in_gp(self, gp_id: str) -> int:
        self.cursor.execute(
            "SELECT COUNT(*) FROM GrandPrixParticipation WHERE grandprix_id = ?",
            (gp_id,),
        )
        return self.cursor.fetchone()[0]
    ...
    def get_player_count_in_tournament(self, t_id: str) -> int:
        self.cursor.execute(
            "SELECT player_count FROM Tournament WHERE tournament_id = ?",
            (t_id,),
        )
        # trying to find it
        try:
    ...
    def get_players_count_eliminated(self, t_id: str) -> int:
        ...
        for gp in gp_ids:
            ...
            if (
                self.get_player_count_in_gp(gp[0]) == 4
                and self.get_race_count_in_gp(gp[0]) == 4
            ):
    ...
    def find_winners_for_gp(
        self, gp_id: str, internal: bool
    ) -> list[tuple[str, int]] | tuple[str, str, str, int]:
    ...
    def find_next_gp_id(self, gp_id: str, t_id: str) -> str:
        ...
        self.cursor.execute(
            "SELECT bracket FROM GrandPrix WHERE grandprix_id = ?",
            (gp_id,),
        )
        # trying to find it
        try:
        ...
        self.cursor.execute(
            "SELECT round FROM GrandPrix WHERE tournament_id = ?",
            (t_id,),
        )
        ...
        self.cursor.execute(
            "SELECT round FROM GrandPrix WHERE grandprix_id = ? AND tournament_id = ?",
            (gp_id, t_id),
        )
        # trying to find it
        try:
            ...
            # if no value then can't subscript
            print(
                f"Error: Tournament ID {t_id} or Grandprix ID {gp_id} not found."
            )
            return ""
    ...
    def insert_gp_results(
        self, gp_id: str, results: list[tuple[str, int]]
    ):
    ...
    def add_winners_to_gp(
        self,
        players: list[tuple[str, str, str, int]],
        gp_id: str,
    ):
        for p in players:
            self.add_player_to_gp(gp_id, p[0], None)

    # finding the tournament winner from the final grand prix id
    def calculate_tournament_winner(
        self, gp_id: str
    ...
    def read_tournament_winner(
        self, t_id: str
    ) -> tuple[str, str, str, int]:
        self.cursor.execute(
            "SELECT * FROM Player WHERE player_id = (SELECT player_id from TournamentParticipation WHERE tournament_id = ? AND tournament_result = 1);",
            (t_id,),
        )
        return self.cursor.fetchone()
    ...
    def get_tournament_result(self, t_id: str, p_id: str) -> int:
        self.cursor.execute(
            "SELECT tournament_result FROM TournamentParticipation WHERE tournament_id = ? AND player_id = ?",
            (t_id, p_id),
        )
        # trying to find it
        try:
            ...
            # if no value then can't subscript
            print(
                f"Error: Tournament ID {t_id} or Player ID {p_id} not found."
            )
            return -1
    ...
    def set_tournament_results(self, t_id: str, force: bool = False):
        ...
        def update_for_round(round: int | None, base: int):
            ...
            self.cursor.execute(
                "SELECT grandprix_id FROM GrandPrix WHERE tournament_id = ? AND round is ?",
                (t_id, round),
            )
            ...
            for gp_id in gp_ids:
                ...
                if round == None:
                    self.cursor.execute(
                        "SELECT * FROM GrandPrixParticipation WHERE grandprix_id = ?",
                        (gp_id,),
                    )
                    ...
                    for p in bottom2:
                        ...
                        self.cursor.execute(
                            "SELECT * FROM GrandPrixParticipation WHERE player_id = ? AND grandprix_id = ?",
                            (p, gp_id),
                        )
            ...
            sorted_round_results = sorted(
                round_results, key=lambda x: x[2]
            )
            ...
            def add_data(
                ...
                # adding the result
                results[base + offset].append(
                    sorted_round_results[position]
                )
                # if the next result has same position then apply recursion
                if (
                    position + 1 < len(sorted_round_results)
                    and sorted_round_results[position + 1][2]
                    == sorted_round_results[position][2]
                ):
            ...
            for pos in results.items():
                for player in pos[1]:
                    self.cursor.execute(
                        "UPDATE TournamentParticipation SET tournament_result = ? WHERE tournament_id = ? AND player_id = ?",
                        (pos[0], t_id, player[1]),
                    )
    ...
    def update_player(
        ...
        self.cursor.execute(
            "UPDATE Player SET forename = ?, surname = ?, age = ? WHERE player_id = ?",
            (forename, surname, age, player_id),
        )
        self.connection.commit()
    ...
    def get_player_circuit_results(
        self, circuit_id: str, player_id: str
    ) -> list[tuple[int, int]]:
    ...
    def get_race_results(self, p_id: str) -> list[int]:
        ...
        self.cursor.execute(
            "SELECT race_result FROM RaceParticipation WHERE player_id = ?",
            (p_id,),
        )
