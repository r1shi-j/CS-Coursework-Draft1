...
class PlayersPage(ttk.Frame):
    ...
    def build_view(self):
        ...
        self.create_btn = AnimatedButton(
            capsule_frame, text="Create Player",
            command=self.open_create_player,
            width=160, height=40, corner_radius=20,
            base_colour=FC.base, hover_colour=FC.purple[1],
            font=FS.base2, hover_cursor="crosshair",
            rounded_corners=["top_left", "bottom_left"]
        )
        ...
        AnimatedButton(
            capsule_frame, text="Edit Player",
            command=self.toggle_edit_mode,
            width=160, height=40, corner_radius=20,
            base_colour=FC.base, hover_colour=FC.gold[1],
            font=FS.base2, hover_cursor="spraycan",
            rounded_corners=["top_right", "bottom_right"]
        ...
        def on_enter():
            self.clear_search.config(foreground=FC.red[1])

        def on_leave():
            self.clear_search.config(foreground=FC.black)

        # only calling the functions is edit_mode is off
        self.clear_search.bind(
            "<Enter>",
            lambda e: on_enter() if not self.edit_mode else None
        )
        self.clear_search.bind(
            "<Leave>",
            lambda e: on_leave() if not self.edit_mode else None
        )
    ...
    def open_create_player(self):
        ...
        ttk.Label(win, text="Surname:").grid(
        ...
        sname_entry.bind(self.controller.CLEAR_TEXT_FIELD, clear_sname)

        # text box for age
        ttk.Label(win, text="Age:").grid(
        ...
        age_entry.bind(self.controller.CLEAR_TEXT_FIELD, clear_age)
        ...
        def create_player():
            ...
            if age < 5 or age > 100:
                messagebox.showerror(
                    "Bad data",
                    "Please enter a reasonable age between 5 and 100",
                )
                return
            ...
            if len(firstname) < 2 or len(firstname) > 35:
                messagebox.showerror(
                    "Bad data",
                    "Please enter a reasonable first name length between 2 and 35 characters",
                )
                return
            if len(surname) < 2 or len(surname) > 35:
                messagebox.showerror(
                    "Bad data",
                    "Please enter a reasonable surname length between 2 and 35 characters",
                )
                return
            ...
            if re.fullmatch(r"[a-zA-Z ]*", firstname) is None:
                messagebox.showerror(
                    "Bad data",
                    "Please enter a firstname using only letters",
                )
                return
            if re.fullmatch(r"[a-zA-Z ]*", surname) is None:
                messagebox.showerror(
                    "Bad data",
                    "Please enter a surname using only letters",
                )
                return
    ...
    def open_edit_player(self, player: tuple[str, str, str, int]):
        ...
        ttk.Label(win, text="Surname:").grid(
            row=1, column=0, padx=(15, 10), pady=8, sticky="e"
        )
        sname_entry = ttk.Entry(win)
        sname_entry.insert(0, player[2])
        sname_entry.grid(
            row=1, column=1, columnspan=2, padx=(5, 20), pady=8
        )
        sname_entry.bind("<Escape>", lambda e: win.focus())
        sname_entry.bind(self.controller.CLEAR_TEXT_FIELD, clear_sname)
        ...
        ttk.Label(win, text="Age:").grid(
            row=2, column=0, padx=(15, 10), pady=8, sticky="e"
        )
        age_entry = ttk.Entry(win)
        age_entry.insert(0, str(player[3]))
        age_entry.grid(
            row=2, column=1, columnspan=2, padx=(5, 20), pady=8
        )
        age_entry.bind("<Escape>", lambda e: win.focus())
        age_entry.bind(self.controller.CLEAR_TEXT_FIELD, clear_age)
        ...
        def update_player():
            ...
            self.controller.db.update_player(
                player[0], firstname, surname, age
            )
        ...
        def delete_player():
            ...
            if confirmed:
                ...
                if did_work:
                    ...
                    messagebox.showerror(
                        "Failed",
                        "Could not delete the player at this time."
                    )
                    return
                win.destroy()
                self.show_results(
                    self.controller.db.read_player_data()
                )
        ...
        AnimatedButton(
            win, text="Delete", command=delete_player,
            hover_cursor="pirate", width=80,
        ...
        AnimatedButton(
            win, text="Update", command=update_player,
            hover_cursor="mouse", width=80,
            base_colour=FC.green[0], hover_colour=FC.green[1]
    ...
    def show_results(self, results: list[tuple[str, str, str, int]]):
        ...
        for row in results:
            ...
            name.bind(
                "<Button-1>",
                lambda e, r=row: (
                    self.open_edit_player(r)
                    if self.edit_mode
                    else self.controller.open_statistics_player(r)
                )
            )
