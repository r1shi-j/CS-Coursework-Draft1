...
class StatisticsPage(ttk.Frame):
    ...
    def build_view(self):
        ...
        buttons_data = [
            (
                ...
                "Displays the players with the most\ntournament wins as a bar chart.",
                FC.blue
            ),
            (
                "Top Rivalries",
                self.show_rivalry,
                "Displays the past tournament results\nfor the top 5 players as a line graph.",
                FC.green
            ),
            (
                "Circuit Popularity",
                self.show_circuit_popularity,
                "Displays the most raced\ncircuits as a pie chart.",
                FC.red
            ),
            (
                "Circuit Analysis",
                self.open_circuit_analysis,
                "Displays who has won a circuit\nthe most times as a bar chart.",
                FC.purple
            ),
            (
                "Player-Circuit Performance",
                self.open_player_circuit_performance,
                "Displays a player's performance\nfor a circuit as a bar chart.",
                FC.gold
            ),
            (
                "Player Analysis",
                self.open_player_analysis,
                "Displays tournament results for a player\nand a box plot showing their consistency.",
                FC.dblue
            )
        ]
        ...
        for text, command, tooltip, hover_colour in buttons_data:
            ...
            btn = AnimatedButton(
                ...
                base_colour=hover_colour[0],
                hover_colour=hover_colour[1]
            )
            btn.pack(pady=10)
    ...
    def show_top_players(self):
        ...
        if not data:
            ...
            tk.Label(
                window,
                text="No data recorded.\nYou haven't finished a tournament yet!",
                font=FS.base2
            ).pack(expand=True)
            return
    ...
    def show_rivalry(self):
        ...
        ax.set_title(
            "Tournament Results Over Time (Top 5 Players)"
        )  # graph title
        ax.set_ylabel("Position")  # y axis title
        ax.set_xlabel("Date")  # x axis title
        ax.xaxis.set_major_formatter(
            mdates.DateFormatter("%d/%m/%y")
        )  # turning datetime objects back to strings
        ...
        ax.set_ylim(
            16.5, 0.5
        )  # so that there is spacing on bottom and top
        ax.legend()  # showing legend
        ax.tick_params(
            axis="x", rotation=45
        )  # rotating x axis data titles
        fig.tight_layout()

        # calling function to show the graph in a new window
        self.open_graph_window("Top Rivalries", fig)
    ...
    def show_circuit_popularity(self):
        ...
        if not data:
            ...
            tk.Label(
                window,
                text="No races recorded.\nYou haven't raced on any circuits yet!",
                font=FS.base2
            ).pack(expand=True)
            return
        ...
        def calculate_sizes(
            ...
            # case 2: there are more than 10 items, so we need to group some into other category
                ...
                cutoff_value = data[split_index][1]
                # count of the 10th item
                ...
                while (
                    split_index > 0
                    and data[split_index - 1][1] == cutoff_value
                ):
                    split_index -= 1
                ...
                other_count = sum(
                    int(row[1]) for row in remaining_data
                )

                # creating the labels and sizes arrays
                labels = [row[0] for row in top_data] + [
                    f"Other ({len(remaining_data)})"
                ]
                sizes = [int(row[1]) for row in top_data] + [
                    other_count
                ]
                return labels, sizes
    ...
    def open_circuit_analysis(
        self, circuit: tuple[str, str] | None = None
    ):
        ...
        if player_count == 0:
            ...
            tk.Label(
                window,
                text="You haven't created any players.\nNo one has raced yet!",
                font=FS.base2
            ).pack(expand=True)
            return
        ...
        combo = ttk.Combobox(
            control_frame, values=circuit_names, state="readonly"
        )
        ...
        combo.bind(
            "<<ComboboxSelected>>",
            lambda e: self.draw_circuit_winners(
                win, combo, circuit_ids
            )
        )
        # if circuit provided then draw it immediately (when hyperlink clicked in circuit view)
        if circuit:
            self.draw_circuit_winners(win, combo, circuit_ids)

    # function to draw the circuit winners graph
    def draw_circuit_winners(
        self,
        win: tk.Toplevel,
        combo: ttk.Combobox,
        ...
        BASE_PATH = (
            f"Mario Kart Images/Resized/{filename}_[] Small.png"
        )
        ...
        load_image_to_label(
            BASE_PATH.replace("[]", "COVER"), win.img_left_label
        )
        load_image_to_label(
            BASE_PATH.replace("[]", "MAP"), win.img_right_label
        )
        ...
        if not data:
            win.error_label = tk.Label(
                win,
                text=f"No one has come 1st place in {c_name} yet.",
                font=FS.base2
            )
        ...
        bars = ax.bar(
            names, wins, color=FC.gold[0], edgecolor=FC.gold[1]
        )
    ...
    def open_player_circuit_performance(self):
        ...
        if player_count == 0:
            ...
            tk.Label(
                window,
                text="You haven't created any players.\nThere isn't any data to view statistics on yet!",
                font=FS.base2
            ).pack(expand=True)
            return
        ...
        ttk.Label(control_frame, text="Circuit:").pack(
        ...
        ttk.Label(control_frame, text="Player:").pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ...
        c_combo.bind(
            "<<ComboboxSelected>>",
            lambda e: self.draw_player_circuit_stats(
                win, c_combo, p_combo, c_ids, p_ids
            )
        )
        p_combo.bind(
            "<<ComboboxSelected>>",
    ...
    def draw_player_circuit_stats(
        ...
        data = self.controller.db.get_player_circuit_results(
            c_id, p_id
        )
        ...
        if not data:
            win.error_label = tk.Label(
                win,
                text=f"No race results found for {p_name} at {c_name}.",
                font=FS.base2
            )
            win.error_label.pack(pady=50, expand=True)
            return
        ...
        if sum(counts) == 0:
            win.error_label = tk.Label(
                win,
                text=f"{p_name} has raced at {c_name},\nbut never finished in the Top 3.",
                font=FS.base2
            )
            win.error_label.pack(pady=50, expand=True)
            return
    ...
    def open_player_analysis(
        self, player: tuple[str, str, str, int] | None = None
    ):
        # getting the number of players, if no player created then showing message
        player_count = self.controller.db.get_player_count()
        if player_count == 0:
            ...
            tk.Label(
                window,
                text="You haven't created any players.\nThere is no data to view analyse yet!",
                font=FS.base2
            ).pack(expand=True)
            return
        ...
        combo.bind(
            "<<ComboboxSelected>>",
            lambda e: self.draw_player_analysis(win, combo, p_ids)
        )
        # if player provided then draw it immediately (when hyperlink clicked in player view)
        if player:
            self.draw_player_analysis(win, combo, p_ids)
    ...
    def draw_player_analysis(
        ...
        ax1.invert_yaxis()
        ax1.set_yticks(
            range(min(results), max(results) + 1)
        )  # showing y axis ticks in range of min and max results values
        ax1.set_ylim(
            max(results) + 0.5, min(results) - 0.5
        )  # so that there is spacing on bottom and top by adding/removing 0.5 to min and max results
        fig1.autofmt_xdate()
        ax1.fmt_xdata = mdates.DateFormatter("%d/%m/%y")
        fig1.tight_layout()
        # adding the line graph to the top frame
        win.canvas_line = FigureCanvasTkAgg(
            fig1, master=win.top_graph_frame
        )
        ...
        if race_results:
            ...
            ax2.boxplot(
                race_results, vert=False, labels=["Race Results"]
            )
            ...
            if len(set(race_results)) == 1:
                ax2.set_xticks(
                    range(race_results[0] - 1, race_results[0] + 2)
                )  # show ticks for ±1 of the result
                ax2.set_xlim(
                    race_results[0] - 1.5, race_results[0] + 1.5
            ...
            win.canvas_box = FigureCanvasTkAgg(
                fig2, master=win.bottom_graph_frame
            )
            win.canvas_box.draw()
            win.canvas_box.get_tk_widget().pack(
